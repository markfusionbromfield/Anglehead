...
properties = {
angleHeadEnabled: {
  title      : "Angle-Head Enabled",
  description: "Enable fixed angle-head remap and plane logic (Auto/±X/±Y).",
  type       : "boolean",
  value      : true,
  group      : "Angle Head"
},
  headOrientation: {
  title      : "Angle-head orientation",
  description: "Auto-detect ±X/±Y from tool vector, or force a specific direction",
  type       : "enum",
  values     : [
    { id:"Auto", title:"Auto-detect" },
    { id:"+X",  title:"+X (tool along +X)" },
    { id:"-X",  title:"-X (tool along -X)" },
    { id:"+Y",  title:"+Y (tool along +Y)" },
    { id:"-Y",  title:"-Y (tool along -Y)" }
  ],
  value      : "Auto",
  group      : "Angle Head"
...
// ================================
var useAngleHead = (function(){ try{ if (getProperty("angleHeadEnabled")===true) return true; }catch(e){} try{ if (getProperty("ah_enable")===true) return true; }catch(e){} return false; })();           // Master switch
var angleHeadAngle = 90;           // 90° offset from main spindle
var angleHeadDirection = "-X";     // default tool direction
try { var __head = getProperty("headOrientation"); if (__head) angleHeadDirection = String(__head); } catch(e) {}
...
function __applyAHOverrides() {
  if (__AH_OVERRIDES_DONE) return;
  try {
    if (typeof machineConfiguration !== 'undefined' && machineConfiguration) {
      if (typeof machineConfiguration.isSupportedDirection === 'function') {
        var __origIsDir = machineConfiguration.isSupportedDirection;
        machineConfiguration.isSupportedDirection = function(direction) {
          try { if (angleHeadActive()) return true; } catch(__) {}
  ...
  try {
    if (typeof getWorkPlaneMachineABC === 'function') {
      var __origGetWP = getWorkPlaneMachineABC;
      getWorkPlaneMachineABC = function(_section, rotate) {
        ...
        return __origGetWP(_section, rotate);
      };
    }
  } catch(__e){}
  try {
    if (typeof setWorkPlaneByMachineABC === 'function') {
      var __origSetWPM = setWorkPlaneByMachineABC;
      setWorkPlaneByMachineABC = function(abc, rotate) {
        try { if (angleHeadActive()) { return; } } catch(__) {}
        return __origSetWPM(abc, rotate);
      };
    }
  } catch(__e){}
  try {
    if (typeof positionABC === 'function') {
      var __origPosABC = positionABC;
      positionABC = function(abc, force) {
        try { if (angleHeadActive()) { return new Vector(0,0,0); } } catch(__) {}
        return __origPosABC(abc, force);
      };
    }
  } catch(__e){}
  try {
    if (typeof setRotation === 'function') {
      var __origSetRot = setRotation;
      setRotation = function(R) {
        try { if (angleHeadActive()) { return; } } catch(__) {}
        return __origSetRot(R);
      };
    }
  } catch(__e){}
  try {
    if (typeof defineWorkPlane === 'function') {
      var __origDefWP = defineWorkPlane;
      defineWorkPlane = function(_section, _setWorkPlane) {
        try { if (angleHeadActive(_section)) { try { forceWorkPlane(); } catch(__) {} return new Vector(0,0,0); } } catch(__) {}
        return __origDefWP(_section, _setWorkPlane);
      };
    }
  } catch(__e){}
...
function resolveAHOrientation(section) {
  // If user forces a direction, honor it; otherwise auto from section workPlane.forward
  try {
    var raw = String((getProperty("headOrientation")||"") + "");
    raw = raw.toUpperCase();
    if (raw && raw !== "AUTO") { return raw; }
  } catch (e) {}
  try {
    var s = section || null;
    var f = s && s.workPlane ? s.workPlane.forward : null;
    if (!f) { throw 0; }
    // pick dominant axis by magnitude of X vs Y
...
function ahAxis(section) {
  ...
  return (dir.indexOf('X') >= 0) ? 'X' : 'Y';
}

function ahSign(section) {
  var dir = resolveAHOrientation(section);
  return (dir.charAt(0) == '-') ? -1 : +1;
}
function isNumber(v) {
...
function angleHeadActive(section) {
  try {
    var en = !!(getProperty("angleHeadEnabled") || getProperty("useAngleHead") || getProperty("ah_enable"));
    if (!en) return false;
    var d = resolveAHOrientation(section);
...
function onOpen() {
  ...
  // === Angle-head -X remap (program Z -> machine X-, X -> Z, Y -> Y; arcs -> G19 with J/K) ===
  if (useAngleHead && angleHeadDirection == "-X") {
    try {
      // Optional: steer the post's plane resolver toward YZ (G19) **if** helpers/variables exist.
      var PLANE_XY = 0, PLANE_ZX = 1, PLANE_YZ = 2;
      if (typeof currentPlane !== "undefined") { currentPlane = PLANE_YZ; }
      if (typeof setCircularPlane === "function") { setCircularPlane(PLANE_YZ); }
      if (typeof gPlaneModal !== "undefined" && typeof writeBlock === "function") { writeBlock(gPlaneModal.format(19)); }

      // Wrap motion emitters to remap coordinates
      if (typeof onRapid === "function") {
        var __onRapid = onRapid;
        onRapid = function(_x, _y, _z) {
          // Mapping chosen: X' = -Zp, Y' = Yp, Z' = Xp
          return __onRapid(-_z, _y, _x);
        };
      }
      if (typeof onLinear === "function") {
        var __onLinear = onLinear;
        onLinear = function(_x, _y, _z, feed) {
          return __onLinear(-_z, _y, _x, feed);
        };
      }
      if (typeof onCircular === "function") {
        var __onCircular = onCircular;
        onCircular = function(clockwise, cx, cy, cz, x, y, z, feed) {
          // Centers remap under same transform:
          //   Cx' = -Cz, Cy' = Cy, Cz' = Cx
          //   X'  = -Z,  Y'  = Y,  Z'  = X
          return __onCircular(clockwise, -cz, cy, cx, -z, y, x, feed);
        };
      }
      if (typeof writeComment === "function") {
        writeComment("Angle head remap active: Z->-X, X->Z, Y->Y (wrapping rapid/linear/circular)");
      }
    } catch (e) {
      if (typeof writeComment === "function") {
        writeComment("Angle head remap hook failed: " + e);
      }
    }
  }


  // Override orientation resolver to avoid internal direction checks
  if (useAngleHead) {
    try {
      if (typeof getWorkPlaneMachineABC === "function") {
        var __getWorkPlaneMachineABC = getWorkPlaneMachineABC;
        getWorkPlaneMachineABC = function(_section, rotate) {
          writeComment("Angle head mode: bypass getWorkPlaneMachineABC");
          return new Vector(0, 0, 0);
        };
      }
      if (typeof defineWorkPlane === "function") {
        // Optionally wrap defineWorkPlane to avoid calls that might force orientation
        var __defineWorkPlane = defineWorkPlane;
        defineWorkPlane = function(_section, _setWorkPlane) {
          // Simply avoid orientation work when angle head is active
          try { forceWorkPlane(); } catch (__) {}
          return new Vector(0, 0, 0);
        };
      }
    } catch (e) {
      writeComment("Note: workplane override hook failed: " + e);
    }
  }


  // Suppress any workplane/rotation output in fixed angle-head mode
  if (useAngleHead) {
    try {
      if (typeof setWorkPlane === "function") {
        var __setWorkPlane = setWorkPlane;
        setWorkPlane = function(abc) {
          writeComment("Angle head mode: suppress setWorkPlane");
          // do not emit any rotation frames
          try {
            if (typeof gRotationModal != "undefined" && gRotationModal.reset) { gRotationModal.reset(); }
          } catch (__) {}
          currentWorkPlaneABC = new Vector(0,0,0);
          return;
        };
      }
      if (typeof forceWorkPlane === "function") {
        var __forceWorkPlane = forceWorkPlane;
        forceWorkPlane = function() {
          writeComment("Angle head mode: suppress forceWorkPlane");
          currentWorkPlaneABC = undefined;
          return;
        };
      }
      if (typeof setRotation === "function") {
        var __setRotation = setRotation;
        setRotation = function(abc) {
          writeComment("Angle head mode: suppress setRotation");
          return;
        };
      }
    } catch (e) {
      writeComment("Note: rotation suppression hook failed: " + e);
    }
  }

  if (useAngleHead) {
    try {
      var dm = (function(){try{return String(getProperty("diagMode")||"off");}catch(e){return "off";}})();
      // accept all orientations in fixed head mode to avoid direction errors
      (function () {
  try {
    ...
    try { var mOn=(function(){try{return getProperty("angleHeadOnCode");}catch(e){return "";}})(); if (mOn) writeBlock(mOn); } catch(e) {}
    try { var dm = String(getProperty("diagMode")||"off"); if (dm!="off") {
      writeComment("=====================================================");
      writeComment(" FIXED ANGLE-HEAD MODE ACTIVE ");
      writeComment(" Orientation: " + angleHeadDirection + " | Offset: 90° ");
      writeComment("=====================================================");
    }} catch(e) {}
  }
...
function onSection() {
  ...
  var ahEnabled = false; try { ahEnabled = (getProperty("angleHeadEnabled")===true) || (getProperty("ah_enable")===true); } catch(e) { ahEnabled = false; }
  try { ahEnabled = getProperty("ah_enable"); } catch(e) { ahEnabled = true; }
